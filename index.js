...
function inject ($, base, cb, opts, ignoredFiles) {
  var items = []
  ...
  if (items.length) {
    ...
    items.forEach(function (el) {
      ...
      if (fs.existsSync(file) && ignoredFiles.indexOf(src) === -1) {
        var toInject = fs.readFileSync(file)
        replace(el, opts.template, toInject)
        done()
      } else {
        done()
      }
...
function inline (html, opts, cb) {
  ...
  typeKeys.forEach(function (type) {
    if (opts.disabledTypes.indexOf(type) === -1) {
      inject($, opts.base, done, typeMap[type], opts.ignore)
    } else {
      done()
    }
  })
...
function replace (el, tmpl, content) {
  el.replaceWith(tmpl(content, el))
}
